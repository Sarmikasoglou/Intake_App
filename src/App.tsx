export default function App() {
  return <div>import React, { useEffect, useMemo, useRef, useState } from "react";
...
type SheetsByDate = Record<string, DaySheet>;

function createDefaultRows(): IntakeRow[] {
...
function formatCalc(value: number | null | undefined): string {
  if (value === null || value === undefined || Number.isNaN(value)) return "";
  return Number(value.toFixed(1)).toString();
}

function calculateFedBinToday(binFedYesterday: number | null, ortsToday: number | null): number | null {
...
function recalcRow(row: IntakeRow): IntakeRow {
...
function normalizeRows(rows: IntakeRow[]): IntakeRow[] {
...
function buildNextDayRows(rows: IntakeRow[]): IntakeRow[] {
  return rows.map((row) =>
    recalcRow({
      ...row,
      cow: row.cow ?? "",
...
function buildAppStatePayload(currentDate: string, sheetsByDate: SheetsByDate, googleDriveFileId: string) {
  return {
    version: 1,
...
function sanitizeImportedState(payload: any) {
  ...
  const normalizedSheets: SheetsByDate = Object.fromEntries(
    Object.entries(payload.sheetsByDate).map(([date, sheet]: [string, any]) => {
...
function downloadCsv(rows: IntakeRow[], date: string, dayOfWeek: string) {
...
function downloadJson(currentDate: string, sheetsByDate: SheetsByDate, googleDriveFileId: string) {
...
async function requestGoogleAccessToken(prompt = "consent"): Promise<string> {
...
function createDriveMultipartRequestBody(metadata: Record<string, unknown>, fileText: string) {
...
async function uploadJsonToGoogleDrive(accessToken: string, payload: any, existingFileId: string) {
  const metadata = {
...
async function downloadJsonFromGoogleDrive(accessToken: string, fileId: string) {
  if (!fileId) {
...
export default function App() {
  ...
  const [sheetsByDate, setSheetsByDate] = useState<SheetsByDate>(() => {
    try {
  ...
  const totals = useMemo(() => {
    const sum = (key: keyof IntakeRow) => rows.reduce((acc, row) => acc + (toNumber(row[key]) ?? 0), 0);
    return {
  ...
  const updateCurrentSheet = (updater: (sheet: DaySheet) => DaySheet) => {
  ...
  const updateRow = (index: number, key: keyof IntakeRow, value: string) => {
  ...
  const updateDayOfWeek = (value: string) => {
